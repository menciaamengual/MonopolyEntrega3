...
public class Juego {
    ...
    public void avanzarCasillas(int avance) {
        if ((jugadorActual.getPosicion() + avance) > 39) {
            ...
            jugadorActual.setFortuna(jugadorActual.getFortuna()+pSalida);
        ...
        switch (entradaPartida[0]) { //Si no reconoce lo introducido, no llamamos a nada, se llama después del switch
                ...
                if (entradaPartida[1].equals("dados")) {
                    ...
                    if (jugadorActual.inCarcel()) {
                        if (jugadorActual.getTurnosCarcel() == 1) {
                            //En el último turno de cárcel, tiene que pagar, y no juega, no puede tirar más veces
                        ...
                        if (jugadorActual.getTurnosCarcel() > 1) {
                            ...
                            if (entradaPartida.length == 5 && entradaPartida[2].equals("trucados")) {
                                ...
                                jugadorActual.setVecesDados(jugadorActual.getVecesDados() + 1);
                            } else {
                                dado.tirarDados();
                                jugadorActual.setVecesDados(jugadorActual.getVecesDados() + 1);
                            }
                        ...
                        if (jugadorActual.getPosicion() != 30) { //Si cae en "ir Carcel" debe ejecutar la acción antes de imprimir el tablero para que el cambio se vea reflejado en la impresión
                            tablero.imprimirTablero();
                            System.out.println("Has caído en "+jugadorActual.getCasilla(tablero.getCasillas())+" .");
                            accionCasilla();
                        } else { // Si no, no es necesario
                            accionCasilla();
                            tablero.imprimirTablero();
                            System.out.println("Has caído en "+jugadorActual.getCasilla(tablero.getCasillas())+" .");
                        }
            ...
            case "acabar":
                if (entradaPartida.length > 1 && entradaPartida[1].equals("turno")) {
                    if ((!haTirado || dado.areEqual())&&!jugadorActual.getBancarrota()) System.out.println("Tienes que tirar antes de terminar el turno");
