...
public class Casilla {
     ...
     */
    public void construir(int tipo,Jugador jugador) {
        if (this.tipo!=0){
            System.out.println("No puedes costruir en una casilla que no sea un solar");
            return;
        }

        if (grupo.getPropietario()!=propietario) { //Comprobación de propiedad
            System.out.println("No puedes construir si no tienes todo el grupo...");
            return;
        }
    ...
    public int calcularAlquiler(Dado dado){
        switch (tipo){
            case 0:
                ...
                else if (grupo.getPropietario()!=null) return 2*alquilerBase;
